...
template <typename Dtype>
__global__ void im2filtercol_gpu_kernel(const int n, const Dtype* data_im, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w,
    const int stride_h, const int stride_w,
    const int height_col, const int width_col,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    int channel_in = h_index / height_col;
	  int channel_out = h_out * width_col + w_out; //chcol in cpu code
    int h_in = h_out * stride_h - pad_h;
    int w_in = w_out * stride_w - pad_w;
		int filter_sz = channels * kernel_h * kernel_w;    

		Dtype* data_col_ptr = data_col;
    data_col_ptr += channel_out * filter_sz + (channel_in * kernel_h) * kernel_w;
    const Dtype* data_im_ptr = data_im;
...
template <typename Dtype>
void im2filtercol_gpu(const Dtype* data_im, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w,
    const int stride_h, const int stride_w,
    Dtype* data_col) {
  ...
  int num_kernels = channels * height_col * width_col;
  // NOLINT_NEXT_LINE(whitespace/operators)
  im2filtercol_gpu_kernel<Dtype><<<CAFFE_GET_BLOCKS(num_kernels),
                             CAFFE_CUDA_NUM_THREADS>>>(
...
// Explicit instantiation
template void im2filtercol_gpu<float>(const float* data_im, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w, const int stride_h, const int stride_w,
    float* data_col);
template void im2filtercol_gpu<double>(const double* data_im, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w, const int stride_h, const int stride_w,
    double* data_col);
...
template <typename Dtype>
__global__ void filtercol2im_gpu_kernel(const int n, const Dtype* data_col,
    const int height, const int width, const int channels,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    */
    // equivalent implementation
		...
		int offset_ch = c * patch_h * patch_w;
    for (int h_col = h_col_start; h_col < h_col_end; ++h_col) {
      for (int w_col = w_col_start; w_col < w_col_end; ++w_col) {
...
template <typename Dtype>
void filtercol2im_gpu(const Dtype* data_col, const int channels,
  ...
  // bottom dimension, and then in the kernel add up the top dimensions.
  // NOLINT_NEXT_LINE(whitespace/operators)
  filtercol2im_gpu_kernel<Dtype><<<CAFFE_GET_BLOCKS(num_kernels),
                             CAFFE_CUDA_NUM_THREADS>>>(
...
// Explicit instantiation
template void filtercol2im_gpu<float>(const float* data_col, const int channels,
    const int height, const int width, const int patch_h, const int patch_w,
    const int pad_h, const int pad_w, const int stride_h,
    const int stride_w, float* data_im);
template void filtercol2im_gpu<double>(const double* data_col, const int channels,
    const int height, const int width, const int patch_h, const int patch_w,
    const int pad_h, const int pad_w, const int stride_h,
