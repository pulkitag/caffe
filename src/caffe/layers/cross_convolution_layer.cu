...
void CrossConvolutionLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
		...
		Dtype* bottom_diff_0 = bottom[0]->mutable_gpu_data();
		...
		for (int n=0; n < num_in_; n++){
			...
			caffe_copy(size_out_, conv_layer_blobs[0]->mutable_gpu_diff(), col_buff);
			...
			if (propagate_down[1]){
				...
				caffe_copy(size_out_, conv_bottom_vec_[0]->mutable_gpu_diff(), bottom_diff_1 + bottom[1]->offset(n));
			}
