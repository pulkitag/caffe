...
template <typename Dtype>
shared_ptr<Layer<Dtype> > GetTopographyLayer(const LayerParameter& param) {
  ...
  if (engine == TopographyParameter_Engine_CAFFE) {
    return shared_ptr<Layer<Dtype> >(new TopographyLayer<Dtype>(param));
#ifdef USE_CUDNN
  } else if (engine == TopographyParameter_Engine_CUDNN) {
    LOG(FATAL) << "Layer " << param.name() << " CUDNN code is not yet written.";
...
//REGISTER_LAYER_CREATOR(Topography, GetTopographyLayer);
*/
