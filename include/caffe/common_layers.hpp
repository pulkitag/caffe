...
/**
 * @brief Compute elementwise operations, such as sum and scale by a constant scalar,
 *
 * TODO(dox): thorough documentation for Forward, Backward, and proto params.
 */
template <typename Dtype>
class GlobalEltwiseLayer : public Layer<Dtype> {
 public:
  explicit GlobalEltwiseLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
...
/**
* @breif: Normalize the blobs by demeaning or scaling by the standard deviation. 
*/
template <typename Dtype>
class NormalizeLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  virtual void Backward_gpu(const vector<Blob<Dtype>*>& top,
	...
	int imSz_;
  NormalizeParameter_NormalizeOp op_;
};
