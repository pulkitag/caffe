...
template <typename Dtype>
class BlockCorrelationLossLayer : public LossLayer<Dtype> {
 public:
   /**
    ...
    */
  explicit BlockCorrelationLossLayer(const LayerParameter& param)
  ...
  virtual inline const char* type() const { return "BlockCorrelationLoss"; }
  virtual inline int ExactNumTopBlobs() const { return 1; }
 ...
 protected:
  ...
  virtual void Forward_gpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  /**
   * @brief Computes the BlockCorrelationLoss.
   *
	 * The BlockCorrelationLoss is defined as following- it is assumed that sequences of images 
	 * are fed into the network. The total number of images if batchSz. Within each batchSz  
 	 * there can be number of blocks, where each block is a sequence. The size of each block is 
	 * the block_size. Each channel of the input is treated seperately and blocks are defined along
	 * the first variable - i.e. number of elements in a batch. 
	 *	
  ...
  bool normalize_;
	...
	std::vector<Blob<Dtype>*> block_data_; 
};
...
template <typename Dtype>
class EuclideanLossWithIgnoreLayer : public LossLayer<Dtype> {
 public:
  explicit EuclideanLossWithIgnoreLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "EuclideanLossWithIgnore"; }
  /**
   * Unlike most loss layers, in the EuclideanLossLayer we can backpropagate
   * to both inputs -- override to return true and always allow force_backward.
   */
 ...
 protected:
  /// @copydoc EuclideanLossWithIgnoreLayer
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Forward_gpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
   ...
   * gradients with respect to the label inputs bottom[1] (but still only will
   * if propagate_down[1] is set, due to being produced by learnable parameters
   * or if force_backward is set). In fact, this layer is NOT "commutative" with 
	 * WithIgnore
   *
   * @param top output Blob vector (length 1), providing the error gradient with
   *      respect to the outputs
